...
class Document:
    ...
    def init_document(self, width: float, height: float) -> None:
    ...
    def set_background(self, color: str) -> None:
        ...
        self.background = self.root.createElement("rect")
        self.background.setAttribute("x", f"-{self.width / 2}")
        self.background.setAttribute("y", f"-{self.height / 2}")
        self.background.setAttribute("width", f"{self.width}")
        self.background.setAttribute("height", f"{self.height}")
        self.background.setAttribute("fill", color)
        self.svg.insertBefore(self.background, self.grid)
    ...
    def add_functions(self, latex_x: str, latex_y: str, padding: int = 10, color: str = "white", size: int = 24):
        ...
        x_label.setAttribute("transform", f"translate({-x_dim[0] / 2}, {position + padding})")
        y_label, y_dim = self.latex_to_svg(latex_y, color=color, size=size)
        y_label.setAttribute("transform", f"translate({-y_dim[0] / 2}, {position + 2 * padding + x_dim[1]})")

        # Adjust frame and backgroudn to new text
        text_heigth = 3 * padding + x_dim[1] + y_dim[1]
        self.svg.setAttribute("height", f"{self.height + text_heigth}px")
        self.svg.setAttribute("viewBox", f"{-self.width / 2} {-self.height / 2} {self.width} {self.height + text_heigth}")

        if self.background_set:
            self.background.setAttribute("height", f"{self.height + text_heigth}")
